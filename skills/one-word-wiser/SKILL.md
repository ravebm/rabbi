...
The need above the gate, the answer below it.
**Free half (250–350 words): the need.** Hebrew script large and centered → transliteration (stress in CAPS) + gloss → "say it" line → the English word and the ache under it (a scene the reader recognizes, "Do you ever…", the cost, the shame taken off; short lines with air) → **one** verse (NIV, then "The word under '…' is *…*", then transliterated Hebrew with the word bolded), sharpening the need, not resolving it → *one line to carry* in italic on its own line (the recognition, said cleanly) → **Today's question**, the one the reader is actually holding → **the gate bridge**: one or two sentences naming where the answer leads, never what it is, and no comfort above the gate.
`` `[PAYWALL]` `` (weeks 1–2: delete the marker; the post runs whole)
**Paid half (250–300 words): the answer.** The turn first, in one breath (the distinction the Hebrew makes that the English can't), then the root and its pictures; exactly one bolded line in the body, here → the secret, 100–160 words: the tradition's evidence, one **hidden layer** of the word that passes the sod test in `02-daily-format.md`, told as a scene, source named in the text, one image the reader keeps (devices welcome, all the tradition's own and sourced; use the bank's secret unless you have a better one; if a day has no real secret, say so instead of shipping a paraphrase) → the payoff (40–60 words, "you know the one") → one thing to do (one sentence, tonight, offered not assigned) → *the line to sleep on* → the close ("Tomorrow: *next word*, gloss.") → "— Rabbi Evan". The paid reader should feel held and surprised, never taught. This is the Already Home model: the beginning stokes a real need, the second half answers it.
