# One Word Wiser — Publication System

**Publication:** rabbi.substack.com → *One Word Wiser*
**Author:** Rabbi Evan Moffic
**Promise:** One Hebrew word, one rabbinic teaching, and one wiser way to live.
**Audience:** Christians (and curious Jews) who want to understand the Bible, Jesus's words, their faith, and themselves more clearly through the Hebrew underneath.

This folder is the operating manual for the relaunch. It is modeled on what is working at
hebrew.substack.com (*Hebrew for Christians* — twice-daily word posts, weekly rhythm,
reader-requested verses, a paid companion kit) and on what already works for Evan at
*Already Home* (daily post, free preview → paid depth, ~2.2% paid conversion on ~19.5k free).

## The one-page version

| | |
|---|---|
| **Cadence** | One post a day, 6:00 am CT, seven days a week. Same shape every day. |
| **☀️ Mon–Fri — *The Word*** | Free half, the need: the word, the English word and the ache under it, one verse with the Hebrew named under the English, one line to carry, today's question, the gate bridge. Paid half, the answer: the turn, the secret inside the word (the *sod*: a gap, a buried root, an enlarged letter, a count the rabbis made), one thing to do tonight, one line to sleep on. 500–650 words in all. |
| **🕯️ Shabbat** | A word from the Torah portion every synagogue reads that morning, whole and free, then the week in five words and the "tell one person" line. |
| **📖 Sunday — *Your Verse*** | A reader's verse, in Hebrew, whole and free. No preview of the week. |
| **Paid tier** | The secret, Monday–Friday. $7/mo · $60/yr ("sixteen cents a day") · Founding $180. 7-day free trial on. Gate turns on in week 3. |
| **The rubric** | The Torah portion is the spine of the week; a holiday, either calendar, overrides it; every word passes four of five tests (familiar, lost in English, teachable, a secret, sayable) and the secret is never the missing one. `word-bank.md` holds a full year. |
| **Growth engine** | Existing list (announcement + 2-week soft start) → Evan's proven Facebook ad → welcome sequence with the *Seven Words* lead magnet → "tell one person" line in every Shabbat post. |
| **Differentiator** | HfC teaches a word. Evan teaches the word's **secret**, the layer the rabbis kept, then one small thing to do with it. And on Shabbat, the portion the whole Jewish world is reading that morning. A pulpit rabbi's move, made for Christians. Nobody else in this lane can do it. |

## Files

| File | What it is |
|---|---|
| `01-positioning.md` | Name decision, tagline, About page, differentiation, voice rules |
| `02-daily-format.md` | Anatomy of the weekday, Shabbat, and Sunday posts; the sod test; subject-line system; cards |
| `03-monetization.md` | Free/paid line, pricing, what paid gets, revenue model, what *not* to do |
| `04-launch-and-growth.md` | 6-week transition plan for the existing list, Facebook ad funnel, welcome sequence, referral loop |
| `05-production-workflow.md` | How we run seven posts a week without it eating Evan's life — the Sunday session, the skill, audio/images, metrics |
| `word-bank.md` | The rubric, the 5787 calendar, and a full year of weeks: five weekday words and a Shabbat word each, with verse and secret |
| `samples/` | Fully written examples: the launch pair, welcome email, the first Sunday preview, Monday (*teshuvah*) in the one-post shape, Shabbat Shuvah (*ha'azinu*) |
| `../skills/one-word-wiser/SKILL.md` | The skill: "write Tuesday's post for *chesed*" or "write the week" → complete posts in this format and voice |

| `operator-brief-substack-setup.md` | Step-by-step Substack configuration with all copy inline, for any agent driving Substack |
| `handoff-prompts.md` | The prompt to paste into Hermes, Codex, or Claude, including the Codex prompt that uploads a week's drafts to Substack |
| `brand/` | Logo files |

All agents read `../AGENTS.md` first — that's what keeps Claude, Hermes, and Codex on the same page.

## How to use this

1. Read `01` → `03` once and decide the open questions flagged in each (they're marked **DECIDE**). Translation policy is set in `01`: Evan's own rendering from the Hebrew for the Hebrew Bible, NIV for the Gospels.
2. Approve or edit the samples in `samples/` — they set the voice for everything the skill generates after.
3. Follow `04-launch-and-growth.md` week by week.
4. Each Sunday, run the skill for the coming week's six words from `word-bank.md`, edit, hand the drafts to Codex to upload, schedule for 6:00 am.
