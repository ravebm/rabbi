# 04 · Launch & Growth

## The advantage and the risk

You start with a large, warm list that subscribed to a *different* newsletter. That list is the whole advantage — and it's also the thing that can go wrong. A sudden jump from one occasional post to twelve a week will unsubscribe the people who liked the old thing. The plan below trades a little speed for keeping them.

Timing note: the relaunch lands on the Days of Awe (Rosh Hashanah begins the evening of **Friday, September 11, 2026**; Yom Kippur the evening of **Sunday, September 20**; verify against a 5787 calendar). There is no better week to start a newsletter about *teshuvah*. Don't wait for a cleaner start.

## Six-week transition

| Week | Dates (2026) | What happens | Cadence |
|---|---|---|---|
| **0** | Sep 8–13 | **Announcement post** to the whole list (`samples/announcement-post.md`). Set up sections (*Morning*, *Evening*), update name/tagline/About/welcome email, build the *Seven Words* PDF, set the paywall to off. Sunday Sep 13: first *Week Ahead* post. | 2 posts |
| **1** | Sep 14–20 | Full rhythm, all free. Theme: *The Days of Awe*. Morning + evening Mon–Fri, Havdalah Saturday, Your Verse + Week Ahead Sunday. Watch unsub rate daily. | 12 posts |
| **2** | Sep 21–27 | Same. Theme: *Atonement* (Yom Kippur Monday). Wednesday: one-line "how's the pace?" question in the morning post; read the replies. Start the Facebook ad Thursday. | 12 |
| **3** | Sep 28–Oct 4 | **Paywall on** (Sunday-evening note). Theme: *Shelter* (Sukkot). Founding window opens. | 12 |
| **4** | Oct 5–11 | Theme: *Beginnings* (Simchat Torah; Torah cycle restarts at Genesis 1). This week is the ad's natural landing content — "the Bible's first word" is the hook. Scale ad spend if CPA holds. | 12 |
| **5** | Oct 12–18 | Theme: *The Image*. First *Ask the Rabbi* live (Thursday 8 pm CT). | 12 + live |
| **6** | Oct 19–25 | Theme: *Covenant*. Founding thank-you post Saturday. Review the numbers (see `05` → Metrics). Decide $60→$70. | 12 |

## Sections (do this in Week 0)

In Substack → Settings → Sections, create two: **Morning** and **Evening**. Assign every weekday post to one. Sunday and Saturday posts go under Morning (the "core") so a reader who mutes Evening still gets the weekly rhythm.

Then in the announcement post and the welcome email, one sentence: *"Two a day is a lot. If you only want mornings, click here and turn off Evening — you'll still get everything free."* Giving readers the off switch is what keeps them from using the unsubscribe switch.

## The Facebook ad funnel

You have the creative that works. The funnel has to be ready to receive it:

```
Facebook ad
  → rabbi.substack.com (Substack's subscribe page, with the About text from 01)
    → immediate welcome email (samples/welcome-email.md) with the Seven Words PDF
      → the next morning post arrives at 6 am — habit begins
        → Day 7: the Saturday recap ("tell one person")
          → Day 14–21: first paywall exposure on an evening post that already fed them
```

**Landing page:** Substack's default subscribe page with the cover image = the word card for *shalom* (Hebrew large, "peace means whole"). The card format *is* the ad creative — the ad and the landing page should look like the same thing.

**Ad copy variants** (headline / body) to test against the one you know works:

1. *"Hope, in Hebrew, is a rope."* / "The Bible wasn't written in English. A rabbi teaches you one Hebrew word a day — and the verse you've read your whole life opens up. Free."
2. *"When Jesus said 'repent,' his listeners heard 'come home.'"* / "One Hebrew word every morning, from Rabbi Evan Moffic, bestselling author of *What Every Christian Needs to Know About the Jewishness of Jesus*. Free, twice a day."
3. *"Read the Bible in the language it was written in."* / "No grammar, no drills. One word, one verse, one rabbi. Two minutes a morning."

Target: interests in Bible study, Christian books, *The Chosen*, Max Lucado / Beth Moore / Tim Keller readers, Israel travel, Messianic Judaism. Women 45–70 are HfC's visible core; your books sell to the same shelf.

**Budget rule:** start at $50/day for five days. If cost per free subscriber < $2.50 and Day-7 open rate of ad-sourced subscribers > 40%, double weekly until either number breaks. Stop scaling at the point where paid conversion of the *ad cohort* (measurable in Substack → Subscribers → filter by source) drops below 1.5% at 60 days.

## Welcome sequence

Substack gives one welcome email. Make it do a lot: the *Seven Words* PDF, what to expect (morning/evening, the off switch), the "send me a verse" invitation, and the one-line paid note at the very bottom. See `samples/welcome-email.md`.

**Seven Words Every Christian Should Know (the lead magnet):** a 7-page PDF, one word per page in the word-card design: *shalom, chesed, emunah, teshuvah, ruach, hallelujah, amen*. Each page: Hebrew, transliteration, root picture, one verse, where Jesus used it, one line to carry. This is ~1,400 words of content that already lives in the word bank. Build it in Week 0; it's the most valuable single asset in the funnel because it's what the ad delivers.

## The referral loop

Three mechanisms, all quiet:

1. **The Saturday line.** Every Havdalah post ends with the same sentence asking readers to tell one person. Ritual, not pitch.
2. **The word card.** Every card has "rabbi.substack.com" in small type in the corner. Cards get screenshotted and posted in church Facebook groups. That's the organic engine.
3. **Substack's own referral program** (Settings → Growth). Rewards: 3 referrals = one month paid; 10 = one year; 25 = a signed book. Mention it once a month, in the Saturday post.

**Notes (Substack's social feed):** post the morning's *one line to carry* as a Note at ~9 am CT, plain text, no link. And the word card image as a Note at ~1 pm. Two Notes a day, both already written. Substack's algorithm currently rewards Hebrew script (it's visually distinct in the feed) — HfC's Notes show it.

## Cross-promotion

- **Recommendations:** turn on Substack recommendations with the two or three publications adjacent to you that are *not* HfC (Christian devotional writers with 20k+; Jewish-Christian dialogue writers). Ask them to recommend back after you have 30 days of the new format live.
- **Already Home:** a single, honest cross-mention in one Already Home post ("I've started a second newsletter…") — that list skews spiritual-seeker, some overlap. Once, not repeated.
- **Podcast guesting:** the pitch is now crisp: "the rabbi who teaches Christians one Hebrew word a day." Pitch five Christian podcasts in Week 4 with the *Seven Words* PDF as the listener gift.
- **Churches:** the group subscription (5+ at 20% off) with a one-page "use *One Word Wiser* in your small group" guide. Sunday-school teachers are the multiplier.

## What to watch in the first 30 days

| Signal | Healthy | Act if |
|---|---|---|
| Unsubscribes / week (existing list) | < 0.7% | > 1.2% → push the sections off-switch harder, consider dropping Friday evening |
| Morning open rate | > 40% | < 30% → subject-line problem; test dropping Hebrew script |
| Evening open rate | > 30% | < 22% → send 8 pm instead of 7; or evening is too long |
| Replies with verses | > 20/week | < 5 → the Sunday ask isn't landing; move it into the morning post once |
| Paid conversions / week (after Week 3) | > 12 | < 6 → the practice section isn't distinct enough from the teaching; audit the gate |
| Ad CPA | < $2.50 | > $4 → pause and swap creative |
